...
const apiUrl = 'covid-193.p.rapidapi.com';
const apiForBelgium = `https://${apiUrl}/history?country=belgium`;
const apiKey = '';
...
async function startRequest() {
	...
	try {
		...
		chrome.browserAction.setBadgeText({
			text: newData.today.newCases.toString(),
		});
	} catch (error) {
		console.error(error);
		localStorage.setItem(storageKey, {
			error: 'An error occured while trying to get the data.',
		});
	} finally {
		console.groupEnd();
	}
...
function getCached(date) {
	...
	try {
		...
		return data &&
			data.datetime &&
			data.datetime.split('T')[0] === date.toISOString().split('T')[0]
			? data
			: null;
	} catch {
		return null;
...
async function getCovidDataAsync(yesterday = false) {
	...
	if (yesterday)
		localStorage.setItem(yesterdayStorageKey, JSON.stringify(formated));
...
async function requestAsync(date) {
	...
	const response = await fetch(
		`${apiForBelgium}&day=${date.toISOString().split('T')[0]}`,
		options
	);
...
function format(data) {
	...
	const newCases = parseInt(data.cases.new);
	const newDeaths = parseInt(data.deaths.new);

	return {
		newCases: isNaN(newCases) ? 'N/A' : newCases,
		newDeaths: isNaN(newDeaths) ? 'N/A' : newDeaths,
